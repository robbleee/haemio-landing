...
function renderDefs(theme: ResolvedTheme): string {
...
function renderNodeShape(ln: LayoutNode, theme: ResolvedTheme): string {
...
function renderNodeLabel(ln: LayoutNode, theme: ResolvedTheme): string {
  ...
  for (let i = 0; i < m.labelLines.length; i++) {
    parts.push(
      `<text x="${cx}" y="${startY + i * lineHeight}" text-anchor="middle" fill="${theme.colors.text}" font-family="${theme.fontFamily}" font-size="${theme.fontSize}" font-weight="600" dominant-baseline="auto">${escapeXml(m.labelLines[i])}</text>`
    );
  }
  ...
  if (m.descLines.length > 0) {
    ...
    for (let i = 0; i < m.descLines.length; i++) {
      parts.push(
        `<text x="${cx}" y="${descStartY + i * descLineHeight}" text-anchor="middle" fill="${theme.colors.edgeLabel}" font-family="${theme.fontFamily}" font-size="${descFontSize}" font-weight="400">${escapeXml(m.descLines[i])}</text>`
      );
    }
  }
...
function renderEdgePath(le: LayoutEdge, theme: ResolvedTheme): string {
  if (le.points.length < 2) return "";
...
function renderTitle(config: FlowConfig, theme: ResolvedTheme, width: number): string {
...
export function renderSVG(config: FlowConfig, layout: LayoutResult): string {
